                ...
                <div class="card-body">
                    <div class="row align-items-center">
                        ...
                        <div class="col-8 text-end">
                            ...
                            <h2 class="text-white">{{ __(optional($user->limitation->package)->name ?? 'Global Package') }}</h2>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="row mt-4">
                                ...
                                <div class="form-group ">
                                      ...
                                      <div class="row mb-3">
                                          <div class="col-md-6">
                                              <div class="form-group">
                                                  ...
                                                  <select name="religion_id" id="religionSelect" class="form-control" required>
                                                      <option value="">@lang('Select One')</option>
                                                      @foreach($religionsList as $rel)
                                                          <option value="{{ $rel->id }}" @selected(optional($user->basicInfo)->religion_id == $rel->id)>{{ __($rel->name) }}</option>
                                                      @endforeach
                                                  </select>
                                              </div>
                                          </div>
                                          <div class="col-md-6">
                                              <div class="form-group">
                                                  <label>@lang('Caste')</label>
                                                  <select name="caste" id="casteSelect" class="form-control">
                                                      <option value="">@lang('Select')</option>
                                                      @foreach($castesList as $cst)
                                                          <option value="{{ $cst->name }}" data-religion="{{ $cst->religion_id }}" @selected(optional($user->basicInfo)->caste == $cst->name)>{{ __($cst->name) }}</option>
                                                      @endforeach
                                                  </select>
                                              </div>
                                          </div>
                                     ...
                                     <!-- Birth Date & Gender -->
                                      <div class="row mb-3">
                                          <div class="col-md-6">
                                              <div class="form-group">
                                                  <label>@lang('Birth Date')</label>
                                                  <input type="date" name="birth_date" class="form-control" value="{{ optional($user->basicInfo)->birth_date ? \Illuminate\Support\Carbon::parse(optional($user->basicInfo)->birth_date)->format('Y-m-d') : '' }}">
                                              </div>
                                          </div>
                                          <div class="col-md-6">
                                              <div class="form-group">
                                                  ...
                                                  <select name="gender" class="form-control">
                                                      <option value="">@lang('Select')</option>
                                                      <option value="m" @selected(optional($user->basicInfo)->gender == 'm')>@lang('Male')</option>
                                                      <option value="f" @selected(optional($user->basicInfo)->gender == 'f')>@lang('Female')</option>
                                                  </select>
                                              </div>
                                          </div>
                                      </div>
                                    ...
                                    <input class="form-control" name="address" type="text" value="{{ @$user->address->address ?? optional(optional($user->basicInfo)->present_address)->address }}">
                                </div>
                            </div>
                            ...
                            <div class="col-xl-3 col-md-6">
                                <div class="form-group">
                                    ...
                                    <input class="form-control" name="city" type="text" value="{{ @$user->address->city ?? optional(optional($user->basicInfo)->present_address)->city }}">
                                </div>
                            </div>
                            ...
                            <div class="col-xl-3 col-md-6">
                                <div class="form-group ">
                                    ...
                                    <input class="form-control" name="state" type="text" value="{{ @$user->address->state ?? optional(optional($user->basicInfo)->present_address)->state }}">
                                </div>
                            </div>
                            ...
                            <div class="col-xl-3 col-md-6">
                                <div class="form-group ">
                                    ...
                                    <input class="form-control" name="zip" type="text" value="{{ @$user->address->zip ?? optional(optional($user->basicInfo)->present_address)->zip }}">
                                </div>
                            </div>
...
@push('script')
    <script>
        (function($) {
            ...
            // Filter caste options based on selected religion
            $('#religionSelect').on('change', function(){
                var relId = $(this).val();
                $('#casteSelect option').each(function(){
                    var rel = $(this).data('religion');
                    if(!relId || rel == relId || $(this).val()===''){
                        $(this).show();
                    }else{
                        $(this).hide();
                    }
