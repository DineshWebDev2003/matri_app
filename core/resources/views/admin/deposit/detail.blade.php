                ...
                <div class="card b-radius--10 overflow-hidden box--shadow1">
                    <div class="card-body">
                        <h5 class="card-title mb-50 border-bottom pb-2">
                            @if ($deposit->purchaseHistory)
                                @lang('User Payment Information for ') - {{ __($deposit->purchaseHistory->package_details->name) }} @lang('Package')
                            @else
                        ...
                        @if ($deposit->status == Status::PAYMENT_PENDING)
                            @can('payments.approve')
                                <div class="row mt-4">
                                    <div class="col-md-12">
                                        <button class="btn btn-outline--success ms-1 confirmationBtn" data-action="{{ route('admin.deposit.approve', $deposit->id) }}" data-question="@lang('Are you sure to approve this transaction?')"><i class="las la-check-double"></i>
                                            @lang('Approve')
                                        </button>

                                        <button class="btn btn-outline--danger ms-1 rejectBtn" data-amount="{{ showAmount($deposit->amount) }} {{ __($general->cur_text) }}" data-id="{{ $deposit->id }}" data-info="{{ $details }}" data-username="{{ @$deposit->user->username }}"><i class="las la-ban"></i> @lang('Reject')
                                        </button>
                                    </div>
                                </div>
                            @endcan
