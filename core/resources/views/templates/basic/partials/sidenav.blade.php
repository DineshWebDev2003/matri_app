...
@push('script')
    <script>
        (function($) {
            ...
            // Cropper variables
            let cropper;
            const fileInput = $('#profilePicUpload1');
            const previewImg = $('<img id="cropperImage" style="max-width:100%;display:none;">');
            // insert preview after input
            fileInput.closest('.thumb').after(previewImg);
            ...
            $('#profilePictureModal form').on('submit', function(ev){
                if (cropper){
                    ev.preventDefault();
