...
class AutomaticGatewayController extends Controller
{
    ...
    public function edit($alias)
    {
        ...
        // Ensure extra config entries are uniform objects so Blade never errors on ->value
        // Ensure $gateway->extra entries are objects with ->value
        if(isset($gateway->extra)){
            $extraArr = is_array($gateway->extra) ? $gateway->extra : (array)$gateway->extra;
            foreach($extraArr as $eKey => $eVal){
                if(is_string($eVal)){
                    $extraArr[$eKey] = (object) [
                        'title' => keyToTitle($eKey),
                        'value' => $eVal,
                    ];
                }elseif(is_array($eVal)){
                    // Cast nested arrays to object for uniformity
                    $extraArr[$eKey] = (object)$eVal;
                }
            }
            $gateway->extra = (object)$extraArr;
        }
        ...
        if ($gateway->currencies->count()) {
            $globalParamRaw = $gateway->currencies->first()->gateway_parameter;
            $decoded = json_decode($globalParamRaw);
            $globalParameters = $decoded ?: (object)[]; // fallback to empty object to avoid null errors when config JSON malformed
            $hasCurrencies = true;
        }
