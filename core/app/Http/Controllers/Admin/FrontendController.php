...
class FrontendController extends Controller
{
    ...
    public function frontendContent(Request $request, $key)
    {
        ...
        if ($type == 'data') {
            ...
            if ($request->hasFile('image_input')) {
                try {
                    ...
                    \Illuminate\Support\Facades\Log::error('SEO image upload failed: ' . $exp->getMessage());
            ...
            if ($imgJson) {
                // Ensure we have an object to read previous values from and avoid "trying to access property on null" errors
                $existingValues = is_object($content->data_values) ? $content->data_values : (object) [];

                foreach ($imgJson as $imgKey => $imgValue) {
                    // Use Laravel helper to safely retrieve uploaded file
                    $imgData = $request->file('image_input.' . $imgKey);
                    if ($imgData instanceof \Illuminate\Http\UploadedFile) {
                        try {
                            $inputContentValue[$imgKey] = $this->storeImage(
                                ...
                                $imgData,
                                $imgKey,
                                $existingValues->$imgKey ?? null
                            );
                        } catch (\Exception $exp) {
                            \Illuminate\Support\Facades\Log::error('Testimonial image upload failed: ' . $exp->getMessage());
                            $notify[] = ['error', "Couldn't upload the image"];
                            return back()->withNotify($notify);
                        }
                    } elseif (property_exists($existingValues, $imgKey)) {
                        // For edit case keep previously stored image if no new file is uploaded
                        $inputContentValue[$imgKey] = $existingValues->$imgKey;
                    }
                }
            }
        }
        ...
        // If SEO content updated, clear cached views & config so changes reflect immediately
        if($key === 'seo'){
            try {
